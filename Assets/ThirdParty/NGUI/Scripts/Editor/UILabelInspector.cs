...
public class UILabelInspector : UIWidgetInspector
{
	...
	protected override bool ShouldDrawProperties ()
	{
		...
		EditorGUI.BeginDisabledGroup(!isValid);
		{
            ...
            SerializedProperty spkey = serializedObject.FindProperty("mKey");
            {
                GUIStyle style = new GUIStyle(EditorStyles.textField);
                ...
                {
                    offset = false;
                    height = style.CalcHeight(new GUIContent(spkey.intValue.ToString()), Screen.width - 20f);
                }
                else
                {
                    GUILayout.BeginHorizontal();
                ...
                GUI.changed = false;
                string value = EditorGUI.TextField(rect, spkey.intValue.ToString(), style);
                if (GUI.changed) spkey.intValue = int.Parse(value);
